...
// Type: Boo.Lang.Environments.ClosedEnvironment
...
namespace Boo.Lang.Environments
{
  public class ClosedEnvironment : IEnvironment
  {
    ...
    public ClosedEnvironment(params object[] bindings) => this._bindings = bindings;

    public TNeed Provide<TNeed>() where TNeed : class
    {
      foreach (object binding in this._bindings)
      {
        if (binding is TNeed need)
          return need;
      }
      return (TNeed) null;
    }
