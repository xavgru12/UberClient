...
public class GameServerConnection : MonoBehaviour
{
	...
	private void OnGUI()
	{
		...
		GUI.Label(new Rect(100f, 30f, 200f, 20f), "Status: " + client.Peer.PeerState.ToString());
		if (client.IsConnected)
		{
			if (GUI.Button(new Rect(100f, 60f, 200f, 20f), "Enter"))
			{
				client.Operations.SendCreateRoom(new GameRoomData
				{
					...
					PlayerLimit = 10,
					KillLimit = 10
				}, string.Empty, ApplicationDataManager.Version, PlayerDataManager.AuthToken, PlayerDataManager.MagicHash, ApplicationDataManager.IsMac);
			}
