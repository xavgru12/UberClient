...
public class TeamEliminationRoom : BaseGameRoom, IDisposable, IGameMode
{
	...
	public GameMode Type => GameMode.TeamElimination;
	...
	public TeamEliminationRoom(GameRoomData gameData, GamePeer peer)
	{
		...
		GameState.Current.Actions.ChangeTeam = base.Operations.SendSwitchTeam;
		...
		EventHandler.Global.AddListener<GlobalEvents.InputChanged>(OnInputChangeEvent);
	}
	...
	public void Dispose()
	{
		if (!isDisposed)
		{
			...
			GameState.Current.PlayerData.SendJumpUpdate -= base.Operations.SendJump;
			GameState.Current.PlayerData.SendMovementUpdate -= base.Operations.SendUpdatePositionAndRotation;
			EventHandler.Global.RemoveListener<GlobalEvents.InputChanged>(OnInputChangeEvent);
	...
	protected override void OnPlayerJoinedGame(GameActorInfo player, PlayerMovement position)
	{
		...
		else if (GameState.Current.MatchState.CurrentStateId != 0 && !player.IsSpectator)
		{
	...
	protected override void OnTeamWins(TeamID team)
	{
		...
		case TeamID.BLUE:
			...
			AutoMonoBehaviour<SfxManager>.Instance.Play2dAudioClip(GameAudio.BlueWins, 0uL);
			break;
		case TeamID.RED:
			GameData.Instance.OnNotification.Fire("Red Team Wins");
			AutoMonoBehaviour<SfxManager>.Instance.Play2dAudioClip(GameAudio.RedWins, 0uL);
			break;
		default:
			GameData.Instance.OnNotification.Fire("Draw!");
			AutoMonoBehaviour<SfxManager>.Instance.Play2dAudioClip(GameAudio.Draw, 0uL);
			break;
		}
	}
