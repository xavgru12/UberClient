...
internal class WindowsUpdater
{
	...
	private static string url_latest = "http://client-steam-qa.uberstrike.com/Entry.txt";

	private static string url_auth = "http://client-steam-qa.uberstrike.com/Auth";

	private static string url_windows = "http://client-steam-qa.uberstrike.com/Windows";
	...
	private static void Finished()
	{
		...
		if (!Application.isEditor)
		{
			Thread thread = new Thread(new ThreadStart(delegate
			{
				int attempts = 0;
				bool spawned = false;
				while (!spawned && attempts < 60)
				{
					try
					{
						Process[] processes = Process.GetProcessesByName("UberStrike");
