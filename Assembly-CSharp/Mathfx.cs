...
public static class Mathfx
{
	...
	{
		return Mathf.Lerp(start, end, (1f + Mathf.Cos(value - (float)Math.PI)) / 2f);
	}
	...
	public static float ProjectedAngle(Vector3 a, Vector3 b)
	{
		...
		if (Vector3.Dot(a, _rotate90 * b) < 0f)
		{
			return 360f - num;
		}
		return num;
	...
	public static float Ease(float t, EaseType easeType)
	{
		...
		{
		case EaseType.In:
			return Mathf.Lerp(0f, 1f, 1f - Mathf.Cos(t * (float)Math.PI * 0.5f));
		case EaseType.Out:
			return Mathf.Lerp(0f, 1f, Mathf.Sin(t * (float)Math.PI * 0.5f));
		case EaseType.InOut:
