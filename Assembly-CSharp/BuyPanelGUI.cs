...
public class BuyPanelGUI : PanelGuiBase
{
	...
	private void DrawUnityItem(Rect rect)
	{
		...
		if (GUI.Button(new Rect(rect.width - (float)num, 0f, num, num), "X", BlueStonez.friends_hidden_button))
		{
			Hide();
		}
	...
	private void DrawTitle(Rect rect)
	{
		...
		GUI.Label(new Rect(58f, 0f, width, 48f), _item.Name, BlueStonez.label_interparkmed_18pt_left_wrap);
		if (_item.View.LevelLock > PlayerDataManager.PlayerLevel)
		{
			...
			GUI.Label(new Rect(rect.width - 10f - 31f - (float)num, 16f, 24f, 24f), _item.View.LevelLock.ToString(), BlueStonez.label_interparkmed_11pt);
	...
	private void DrawBuyButton(Rect rect)
	{
		...
		if (GUI.Button(position, GUIContent.none, BlueStonez.buttongold_large) && !_isBuyingItem)
		{
			_isBuyingItem = true;
		...
		GUI.Label(position2, new GUIContent(_priceTag, _priceIcon), BlueStonez.label_interparkbold_13pt_black);
		GUI.EndGroup();
	}

	private void OnPriceOptionSelected(ItemPrice price)
	{
		_priceTag = ((price.Price != 0) ? $"{price.Price:N0}" : "FREE");
	...
	public static void BuyItem(IUnityItem item, ItemPrice price, BuyingLocationType buyingLocation = BuyingLocationType.Shop, BuyingRecommendationType recommendation = BuyingRecommendationType.Manual, bool autoEquip = false)
	{
		if (item.View.IsConsumable)
		{
			...
			{
				HandleBuyItem(item, (BuyItemResult)result, autoEquip);
			}, delegate
			{
				_isBuyingItem = false;
	...
	private static void HandleBuyItem(IUnityItem item, BuyItemResult result, bool autoEquip)
	{
		...
		case BuyItemResult.PackDisabled:
			...
			break;
		case BuyItemResult.InvalidLevel:
