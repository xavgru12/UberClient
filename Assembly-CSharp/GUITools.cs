...
public static class GUITools
{
	...
	public static float AspectRatio => _aspectRatio;

	public static float SinusPulse => (Mathf.Sin(Time.time * 2f) + 1.3f) * 0.5f;

	public static float FastSinusPulse => (Mathf.Sin(Time.time * 5f) + 1.3f) * 0.5f;

	public static int ScreenHalfWidth => _screenHalfX;

	public static int ScreenHalfHeight => _screenHalfY;
	...
	public static float LastClick => _lastClick;

	public static bool SaveClick => _lastClick + 0.5f < Time.time;
	...
	public static bool RepeatClick(float vel)
	{
		...
		if (_repeatButtonPressed != 0f)
		{
			if (_repeatButtonPressed > 0.5f)
			{
				return SaveClickIn(vel);
			}
			return false;
		}
		return true;
	...
	public static bool Button(Rect rect, GUIContent content, GUIStyle style, AudioClip soundEffect)
	{
		...
		{
			AutoMonoBehaviour<SfxManager>.Instance.Play2dAudioClip(soundEffect, 0uL);
			return true;
		}
		return false;
	}
	...
	{
		return BeginScrollView(position, scrollPosition, contentRect, showHorizontalScrollbar: false, showVerticalScrollbar: false, GUI.skin.horizontalScrollbar, GUI.skin.verticalScrollbar);
	}
