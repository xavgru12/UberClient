...
public class ArmorBuffQuickItem : QuickItem
{
  ...
  public override QuickItemConfiguration Configuration
  {
    ...
    set => this._config = (ArmorBuffConfiguration) value;
  }

  protected override void OnActivated()
  {
    if (!this.machine.ContainsState(1))
      this.machine.RegisterState(1, (IState) new ArmorBuffQuickItem.ActivatedState(this));
    Singleton<QuickItemSfxController>.Instance.ShowThirdPersonEffect(GameState.LocalPlayer.Character, QuickItemLogic.ArmorPack, this._config.RobotLifeTimeMilliSeconds, this._config.ScrapsLifeTimeMilliSeconds, this._config.IsInstant);
    GameState.CurrentGame.ActivateQuickItem(QuickItemLogic.ArmorPack, this._config.RobotLifeTimeMilliSeconds, this._config.ScrapsLifeTimeMilliSeconds, this._config.IsInstant);
  ...
  private void OnGUI()
  {
    ...
    GUI.Label(new Rect((float) (((double) Screen.width - (double) width) * 0.5), (float) (Screen.height / 2 + 20), width, height), "Charging Armor", BlueStonez.label_interparkbold_16pt);
  ...
  private class ActivatedState : IState
  {
    ...
    public ActivatedState(ArmorBuffQuickItem configuration) => this._item = configuration;
    ...
    private void SendHealthIncrease()
    {
      ...
      switch (this._item._config.ArmorIncrease)
      {
        ...
        case IncreaseStyle.PercentFromMax:
          ...
          break;
        default:
          throw new NotImplementedException("SendArmorIncrease for type: " + (object) this._item._config.ArmorIncrease);
      }
      CmuneEventHandler.Route((object) new ArmorIncreaseEvent()
      {
        Armor = pointsGain
      });
    }
