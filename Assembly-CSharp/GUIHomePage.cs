...
internal class GUIHomePage : GUIPageBase
{
	...
	{
		float dismissDuration = base.dismissDuration;
		if (ApplicationDataManager.IsDesktop)
		{
			yield return StartCoroutine(AnimateAlpha(0f, dismissDuration, profileButton, clansButton, chatButton, inboxButton, shopButton, playButton, quitButton));
		}
		else
		{
			yield return StartCoroutine(AnimateAlpha(0f, dismissDuration, profileButton, clansButton, chatButton, inboxButton, shopButton, playButton));
		}
	}
}
