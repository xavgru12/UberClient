...
public class ShopItemGrid
{
	...
	public List<bool> HighlightState
	{
		get;
		set;
	}
	...
	public List<ShopItemView> Items => _items;
	...
	public ShopItemGrid(List<BundleItemView> items, int credits = 0, int points = 0)
	{
		...
		{
			_items.Add(new ShopItemView(item));
		}
		if (credits > 0)
		{
	...
	public void Draw(Rect rect)
	{
		...
		int num2 = _items.Count / 6 + ((_items.Count % 6 > 0) ? 1 : 0);
		...
		for (int i = 0; i < num2; i++)
		{
			...
			{
				int num3 = i * 6 + j;
				Rect rect2 = new Rect((float)j * num, rect.height - (float)(i + 1) * num - offset, num, num);
