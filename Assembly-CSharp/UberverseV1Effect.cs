...
public sealed class UberverseV1Effect : MonoBehaviour
{
    // Next unused ID after PR #8's retired 9068-9078 range. Catalog must use this same ID.
    public const int ItemId = 9083;
    public const string RootName = "UberverseV1_OrbitalSystem";
    ...
    public static bool Owns(Renderer renderer)
    {
        ...
        for (Transform t = renderer.transform; t != null; t = t.parent)
            if (t.GetComponent<UberverseV1Effect>() != null) return true;
        return false;
    }
    ...
    public static void Apply(GameObject weaponRoot, int itemId)
    {
        ...
        UberverseV1Effect[] existing = weaponRoot.GetComponentsInChildren<UberverseV1Effect>(true);
        bool retained = false;
        foreach (UberverseV1Effect effect in existing)
        {
        ...
        foreach (MeshFilter filter in weaponRoot.GetComponentsInChildren<MeshFilter>(true))
        {
            ...
            UberverseV1Effect effect = root.AddComponent<UberverseV1Effect>();
    ...
    private void Initialize(Renderer body, Bounds bounds)
    {
        ...
        if (additive != null)
        {
            ...
            paths.gameObject.AddComponent<UberverseV1RibbonCamera>().Owner = this;
    ...
    {
        float radius = (.052f + i * .019f) * scale;
        Vector3 p = new Vector3(Mathf.Cos(angle) * radius,
            Mathf.Sin(angle) * (.014f + i * .003f) * scale,
            Mathf.Sin(angle) * (.078f + i * .013f) * scale);
        return anchor + p + new Vector3(0f, i * .018f * scale, (i - 1) * .018f * scale);
    }
    ...
    private void Animate(double time)
    {
        ...
        for (int i = 0; i < PlanetCount; i++)
        {
            ...
            spriteSizes[i] = PlanetRadius(i) * 2.4f;
            spriteColours[i] = WithAlpha(Color.Lerp(Palette[i], Cyan, .22f), .26f);
        }
        ...
        for (int j = 0; j < RibbonSegments; j++)
        {
            float a = j * Mathf.PI * 2f / RibbonSegments;
            float b = (j + 1) * Mathf.PI * 2f / RibbonSegments;
            Vector3 p = tilt * new Vector3(Mathf.Cos(a), 0f, Mathf.Sin(a)) * .037f * scale;
            Vector3 q = tilt * new Vector3(Mathf.Cos(b), 0f, Mathf.Sin(b)) * .037f * scale;
        ...
        for (int i = 3; i < SpriteCount; i++)
        {
            ...
            if (i < 12)
            {
                ...
                spritePositions[i] = anchor + new Vector3(Mathf.Cos(phase) * .055f,
                    -.10f + Mathf.Sin(phase) * .025f, -.19f + n * .06f) * scale;
                spriteSizes[i] = (.085f + .016f * wave) * scale;
                spriteColours[i] = WithAlpha(Palette[n % 3], .055f + wave * .020f);
            }
            else
            {
                spritePositions[i] = anchor + new Vector3(Mathf.Cos(phase) * (.09f + n % 4 * .018f),
                    -.045f + Mathf.Sin(Phase(time, .14, n * 2.399963 * 1.4)) * .10f,
                    -.22f + (n % 13) * .044f) * scale;
                spriteSizes[i] = (.0025f + .0015f * wave + (n % 9 == 0 ? .002f : 0f)) * scale;
...
public sealed class UberverseV1RibbonCamera : MonoBehaviour
{
    public UberverseV1Effect Owner;
    private void OnWillRenderObject()
    {
