...
namespace System.Xml.Schema
{
  public class XmlSchemaLengthFacet : XmlSchemaNumericFacet
  {
    ...
    internal override XmlSchemaFacet.Facet ThisFacet => XmlSchemaFacet.Facet.length;

    internal static XmlSchemaLengthFacet Read(XmlSchemaReader reader, ValidationEventHandler h)
    {
      XmlSchemaLengthFacet xso = new XmlSchemaLengthFacet();
      reader.MoveToElement();
      if (reader.NamespaceURI != "http://www.w3.org/2001/XMLSchema" || reader.LocalName != "length")
      {
        XmlSchemaObject.error(h, "Should not happen :1: XmlSchemaLengthFacet.Read, name=" + reader.Name, (Exception) null);
        reader.Skip();
        return (XmlSchemaLengthFacet) null;
      }
      ...
      while (reader.ReadNextElement())
      {
        if (reader.NodeType == XmlNodeType.EndElement)
        {
          ...
          {
            XmlSchemaObject.error(h, "Should not happen :2: XmlSchemaLengthFacet.Read, name=" + reader.Name, (Exception) null);
            break;
          }
          break;
        }
