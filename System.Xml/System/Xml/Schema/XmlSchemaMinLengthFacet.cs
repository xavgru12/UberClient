...
namespace System.Xml.Schema
{
  public class XmlSchemaMinLengthFacet : XmlSchemaNumericFacet
  {
    ...
    internal override XmlSchemaFacet.Facet ThisFacet => XmlSchemaFacet.Facet.minLength;

    internal static XmlSchemaMinLengthFacet Read(XmlSchemaReader reader, ValidationEventHandler h)
    {
      XmlSchemaMinLengthFacet xso = new XmlSchemaMinLengthFacet();
      reader.MoveToElement();
      if (reader.NamespaceURI != "http://www.w3.org/2001/XMLSchema" || reader.LocalName != "minLength")
      {
        XmlSchemaObject.error(h, "Should not happen :1: XmlSchemaMinLengthFacet.Read, name=" + reader.Name, (Exception) null);
        reader.Skip();
        return (XmlSchemaMinLengthFacet) null;
      }
      ...
      while (reader.MoveToNextAttribute())
      {
        ...
        else if (reader.Name == "fixed")
        {
          ...
          XmlSchemaObject.error(h, reader.Name + " is not a valid attribute for minLength", (Exception) null);
        else
      ...
      while (reader.ReadNextElement())
      {
        if (reader.NodeType == XmlNodeType.EndElement)
        {
          ...
          {
            XmlSchemaObject.error(h, "Should not happen :2: XmlSchemaMinLengthFacet.Read, name=" + reader.Name, (Exception) null);
            break;
          }
          break;
        }
