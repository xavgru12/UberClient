...
namespace System.Xml.Schema
{
  public class XmlSchemaTotalDigitsFacet : XmlSchemaNumericFacet
  {
    ...
    internal override XmlSchemaFacet.Facet ThisFacet => XmlSchemaFacet.Facet.totalDigits;

    internal static XmlSchemaTotalDigitsFacet Read(XmlSchemaReader reader, ValidationEventHandler h)
    {
      XmlSchemaTotalDigitsFacet xso = new XmlSchemaTotalDigitsFacet();
      reader.MoveToElement();
      if (reader.NamespaceURI != "http://www.w3.org/2001/XMLSchema" || reader.LocalName != "totalDigits")
      {
        XmlSchemaObject.error(h, "Should not happen :1: XmlSchemaTotalDigitsFacet.Read, name=" + reader.Name, (Exception) null);
        reader.Skip();
        return (XmlSchemaTotalDigitsFacet) null;
      }
      ...
      while (reader.MoveToNextAttribute())
      {
        ...
        else if (reader.Name == "fixed")
        {
          ...
          XmlSchemaObject.error(h, reader.Name + " is not a valid attribute for totalDigits", (Exception) null);
        else
      ...
      while (reader.ReadNextElement())
      {
        if (reader.NodeType == XmlNodeType.EndElement)
        {
          ...
          {
            XmlSchemaObject.error(h, "Should not happen :2: XmlSchemaTotalDigitsFacet.Read, name=" + reader.Name, (Exception) null);
            break;
          }
          break;
        }
