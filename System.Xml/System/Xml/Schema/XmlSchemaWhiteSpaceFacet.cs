...
namespace System.Xml.Schema
{
  public class XmlSchemaWhiteSpaceFacet : XmlSchemaFacet
  {
    ...
    internal override XmlSchemaFacet.Facet ThisFacet => XmlSchemaFacet.Facet.whiteSpace;

    internal static XmlSchemaWhiteSpaceFacet Read(XmlSchemaReader reader, ValidationEventHandler h)
    {
      XmlSchemaWhiteSpaceFacet xso = new XmlSchemaWhiteSpaceFacet();
      reader.MoveToElement();
      if (reader.NamespaceURI != "http://www.w3.org/2001/XMLSchema" || reader.LocalName != "whiteSpace")
      {
        XmlSchemaObject.error(h, "Should not happen :1: XmlSchemaWhiteSpaceFacet.Read, name=" + reader.Name, (Exception) null);
        reader.Skip();
        return (XmlSchemaWhiteSpaceFacet) null;
      }
      ...
      while (reader.MoveToNextAttribute())
      {
        ...
        else if (reader.Name == "fixed")
        {
          ...
          XmlSchemaObject.error(h, reader.Name + " is not a valid attribute for whiteSpace", (Exception) null);
        else
      ...
      while (reader.ReadNextElement())
      {
        if (reader.NodeType == XmlNodeType.EndElement)
        {
          ...
          {
            XmlSchemaObject.error(h, "Should not happen :2: XmlSchemaWhiteSpaceFacet.Read, name=" + reader.Name, (Exception) null);
            break;
          }
          break;
        }
