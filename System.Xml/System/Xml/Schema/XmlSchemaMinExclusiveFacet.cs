...
namespace System.Xml.Schema
{
  public class XmlSchemaMinExclusiveFacet : XmlSchemaFacet
  {
    ...
    internal override XmlSchemaFacet.Facet ThisFacet => XmlSchemaFacet.Facet.minExclusive;

    internal static XmlSchemaMinExclusiveFacet Read(
      XmlSchemaReader reader,
      ValidationEventHandler h)
    {
      XmlSchemaMinExclusiveFacet xso = new XmlSchemaMinExclusiveFacet();
      reader.MoveToElement();
      if (reader.NamespaceURI != "http://www.w3.org/2001/XMLSchema" || reader.LocalName != "minExclusive")
      {
        XmlSchemaObject.error(h, "Should not happen :1: XmlSchemaMinExclusiveFacet.Read, name=" + reader.Name, (Exception) null);
        reader.Skip();
        return (XmlSchemaMinExclusiveFacet) null;
      }
      ...
      while (reader.MoveToNextAttribute())
      {
        ...
        else if (reader.Name == "fixed")
        {
          ...
          XmlSchemaObject.error(h, reader.Name + " is not a valid attribute for minExclusive", (Exception) null);
        else
      ...
      while (reader.ReadNextElement())
      {
        if (reader.NodeType == XmlNodeType.EndElement)
        {
          ...
          {
            XmlSchemaObject.error(h, "Should not happen :2: XmlSchemaMinExclusiveFacet.Read, name=" + reader.Name, (Exception) null);
            break;
          }
          break;
        }
