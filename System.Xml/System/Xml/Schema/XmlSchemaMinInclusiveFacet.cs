...
namespace System.Xml.Schema
{
  public class XmlSchemaMinInclusiveFacet : XmlSchemaFacet
  {
    ...
    internal override XmlSchemaFacet.Facet ThisFacet => XmlSchemaFacet.Facet.minInclusive;

    internal static XmlSchemaMinInclusiveFacet Read(
      XmlSchemaReader reader,
      ValidationEventHandler h)
    {
      XmlSchemaMinInclusiveFacet xso = new XmlSchemaMinInclusiveFacet();
      reader.MoveToElement();
      if (reader.NamespaceURI != "http://www.w3.org/2001/XMLSchema" || reader.LocalName != "minInclusive")
      {
        XmlSchemaObject.error(h, "Should not happen :1: XmlSchemaMinInclusiveFacet.Read, name=" + reader.Name, (Exception) null);
        reader.Skip();
        return (XmlSchemaMinInclusiveFacet) null;
      }
      ...
      while (reader.MoveToNextAttribute())
      {
        ...
        else if (reader.Name == "fixed")
        {
          ...
          XmlSchemaObject.error(h, reader.Name + " is not a valid attribute for minInclusive", (Exception) null);
        else
      ...
      while (reader.ReadNextElement())
      {
        if (reader.NodeType == XmlNodeType.EndElement)
        {
          ...
          {
            XmlSchemaObject.error(h, "Should not happen :2: XmlSchemaMinInclusiveFacet.Read, name=" + reader.Name, (Exception) null);
            break;
          }
          break;
        }
