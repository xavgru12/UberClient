...
// Type: Mono.Xml.Schema.XsdUnsignedInt
...
namespace Mono.Xml.Schema
{
  internal class XsdUnsignedInt : XsdUnsignedLong
  {
    ...
    internal override System.ValueType ParseValueType(
      ...
      IXmlNamespaceResolver nsmgr)
    {
      return (System.ValueType) XmlConvert.ToUInt32(this.Normalize(s));
    }

    internal override XsdOrdering Compare(object x, object y)
    {
      if (!(x is uint) || !(y is uint num))
        return XsdOrdering.Indeterminate;
      if ((int) (uint) x == (int) num)
        return XsdOrdering.Equal;
      return (uint) x < (uint) y ? XsdOrdering.LessThan : XsdOrdering.GreaterThan;
    }
  }
}
