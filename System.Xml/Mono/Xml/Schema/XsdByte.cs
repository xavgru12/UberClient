...
namespace Mono.Xml.Schema
{
  internal class XsdByte : XsdShort
  {
    ...
    internal override System.ValueType ParseValueType(
      ...
      IXmlNamespaceResolver nsmgr)
    {
      return (System.ValueType) XmlConvert.ToSByte(this.Normalize(s));
    }

    internal override XsdOrdering Compare(object x, object y)
    {
      if (!(x is sbyte) || !(y is sbyte num))
        return XsdOrdering.Indeterminate;
      if ((int) (sbyte) x == (int) num)
        return XsdOrdering.Equal;
      return (int) (sbyte) x < (int) (sbyte) y ? XsdOrdering.LessThan : XsdOrdering.GreaterThan;
    }
  }
}
