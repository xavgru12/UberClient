...
// Type: Mono.Xml.Schema.XsdDuration
...
namespace Mono.Xml.Schema
{
  ...
  {
    internal XsdDuration() => this.WhitespaceValue = XsdWhitespaceFacet.Collapse;
    ...
    {
      if (!(x is TimeSpan) || !(y is TimeSpan t2))
        return XsdOrdering.Indeterminate;
      int num = TimeSpan.Compare((TimeSpan) x, t2);
      if (num < 0)
