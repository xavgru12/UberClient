  ...
  public static class AuthenticationWebServiceClient
  {
      ...
      Action<Exception> handler)
    {
      using (MemoryStream bytes = new MemoryStream())
      {
        ...
        EnumProxy<ChannelType>.Serialize((Stream) bytes, channelType);
        StringProxy.Serialize((Stream) bytes, machineId);
        return MonoInstance.Mono.StartCoroutine(SoapClient.MakeRequest("IAuthenticationWebServiceContract", "AuthenticationWebService", nameof (LoginMemberCookie), bytes.ToArray(), (Action<byte[]>) (data =>
        {
          if (callback == null)
            return;
          callback(MemberAuthenticationResultViewProxy.Deserialize((Stream) new MemoryStream(data)));
      ...
      Action<Exception> handler)
    {
      using (MemoryStream bytes = new MemoryStream())
      {
        ...
        StringProxy.Serialize((Stream) bytes, hash);
        StringProxy.Serialize((Stream) bytes, machineId);
        return MonoInstance.Mono.StartCoroutine(SoapClient.MakeRequest("IAuthenticationWebServiceContract", "AuthenticationWebService", nameof (LoginMemberFacebook), bytes.ToArray(), (Action<byte[]>) (data =>
        {
          if (callback == null)
            return;
          callback(MemberAuthenticationResultViewProxy.Deserialize((Stream) new MemoryStream(data)));
      ...
      Action<Exception> handler)
    {
      using (MemoryStream bytes = new MemoryStream())
      {
        ...
        EnumProxy<ChannelType>.Serialize((Stream) bytes, channelType);
        StringProxy.Serialize((Stream) bytes, machineId);
        return MonoInstance.Mono.StartCoroutine(SoapClient.MakeRequest("IAuthenticationWebServiceContract", "AuthenticationWebService", nameof (FacebookSingleSignOn), bytes.ToArray(), (Action<byte[]>) (data =>
        {
          if (callback == null)
            return;
          callback(MemberAuthenticationResultViewProxy.Deserialize((Stream) new MemoryStream(data)));
