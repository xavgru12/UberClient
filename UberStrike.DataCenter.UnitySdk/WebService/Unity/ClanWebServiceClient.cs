  ...
  public static class ClanWebServiceClient
  {
      ...
      Action<Exception> handler)
    {
      using (MemoryStream bytes = new MemoryStream())
      {
        ...
        return MonoInstance.Mono.StartCoroutine(SoapClient.MakeRequest("IClanWebServiceContract", "ClanWebService", nameof (IsMemberPartOfGroup), bytes.ToArray(), (Action<byte[]>) (data =>
        {
          if (callback == null)
            return;
      ...
      Action<Exception> handler)
    {
      using (MemoryStream bytes = new MemoryStream())
      {
        ...
        return MonoInstance.Mono.StartCoroutine(SoapClient.MakeRequest("IClanWebServiceContract", "ClanWebService", nameof (IsMemberPartOfAnyGroup), bytes.ToArray(), (Action<byte[]>) (data =>
        {
          if (callback == null)
            return;
          callback(BooleanProxy.Deserialize((Stream) new MemoryStream(data)));
      ...
      Action<Exception> handler)
    {
      ...
      {
        Int32Proxy.Serialize((Stream) bytes, groupId);
        return MonoInstance.Mono.StartCoroutine(SoapClient.MakeRequest("IClanWebServiceContract", "ClanWebService", nameof (GetClan), bytes.ToArray(), (Action<byte[]>) (data =>
        {
          if (callback == null)
            return;
