  ...
  public static class RelationshipWebServiceClient
  {
      ...
      Action<Exception> handler)
    {
      using (MemoryStream bytes = new MemoryStream())
      {
        ...
        return MonoInstance.Mono.StartCoroutine(SoapClient.MakeRequest("IRelationshipWebServiceContract", "RelationshipWebService", nameof (SendContactRequest), bytes.ToArray(), (Action<byte[]>) (data =>
        {
          if (callback == null)
            return;
      ...
      Action<Exception> handler)
    {
      using (MemoryStream bytes = new MemoryStream())
      {
        ...
        return MonoInstance.Mono.StartCoroutine(SoapClient.MakeRequest("IRelationshipWebServiceContract", "RelationshipWebService", nameof (MoveContactToGroup), bytes.ToArray(), (Action<byte[]>) (data =>
        {
          if (callback == null)
            return;
          callback(EnumProxy<MemberOperationResult>.Deserialize((Stream) new MemoryStream(data)));
