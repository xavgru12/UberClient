	...
	public static class ClanViewProxy
	{
		public static void Serialize(Stream stream, ClanView instance)
		{
			...
			using (MemoryStream memoryStream = new MemoryStream())
			{
				...
				{
					ListProxy<ClanMemberView>.Serialize(memoryStream, instance.Members, ClanMemberViewProxy.Serialize);
				}
				else
				{
					num |= 4;
		...
		public static ClanView Deserialize(Stream bytes)
		{
			...
			if ((num & 4) != 0)
			{
				clanView.Members = ListProxy<ClanMemberView>.Deserialize(bytes, ClanMemberViewProxy.Deserialize);
			}
