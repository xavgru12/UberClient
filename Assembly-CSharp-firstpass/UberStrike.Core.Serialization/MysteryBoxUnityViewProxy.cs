	...
	public static class MysteryBoxUnityViewProxy
	{
		public static void Serialize(Stream stream, MysteryBoxUnityView instance)
		{
			...
			using (MemoryStream memoryStream = new MemoryStream())
			{
				...
				{
					ListProxy<BundleItemView>.Serialize(memoryStream, instance.MysteryBoxItems, BundleItemViewProxy.Serialize);
				}
				else
				{
					num |= 8;
		...
		public static MysteryBoxUnityView Deserialize(Stream bytes)
		{
			...
			if ((num & 8) != 0)
			{
				mysteryBoxUnityView.MysteryBoxItems = ListProxy<BundleItemView>.Deserialize(bytes, BundleItemViewProxy.Deserialize);
			}
			if ((num & 0x10) != 0)
			{
