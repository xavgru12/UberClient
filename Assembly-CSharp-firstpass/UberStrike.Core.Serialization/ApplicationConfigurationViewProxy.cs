	...
	public static class ApplicationConfigurationViewProxy
	{
		public static void Serialize(Stream stream, ApplicationConfigurationView instance)
		{
			...
			using (MemoryStream memoryStream = new MemoryStream())
			{
				...
				{
					DictionaryProxy<int, int>.Serialize(memoryStream, instance.XpRequiredPerLevel, Int32Proxy.Serialize, Int32Proxy.Serialize);
				}
				else
				{
					num |= 1;
		...
		public static ApplicationConfigurationView Deserialize(Stream bytes)
		{
			...
			if ((num & 1) != 0)
			{
				applicationConfigurationView.XpRequiredPerLevel = DictionaryProxy<int, int>.Deserialize(bytes, Int32Proxy.Deserialize, Int32Proxy.Deserialize);
			}
