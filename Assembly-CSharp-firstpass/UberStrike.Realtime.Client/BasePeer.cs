...
namespace UberStrike.Realtime.Client
{
	public abstract class BasePeer : IDisposable
	{
		...
		public int ServerTimeTicks => Peer.ServerTimeInMilliSeconds & int.MaxValue;

		public bool IsConnected => Peer.PeerState == PeerStateValue.Connected;
		...
		protected BasePeer(int syncFrequency, bool monitorTraffic)
		{
			...
			SyncFrequency = (float)syncFrequency / 1000f;
			Monitor = new TrafficMonitor(monitorTraffic);
			IsEnabled = true;
			if (monitorTraffic)
			{
				listener.OnError += delegate(string error)
				{
					Monitor.AddEvent(error);
				};
				listener.OnConnect += delegate
				{
					Monitor.AddEvent("Connected");
				};
				listener.OnDisconnect += delegate
				{
					Monitor.AddEvent("Disconnected");
				};
