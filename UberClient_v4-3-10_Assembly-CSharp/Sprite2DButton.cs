...
public class Sprite2DButton : IAnimatable2D
{
  ...
  {
    this.IsUsingGuiContentBounds = true;
  ...
  public void Draw(float offsetX = 0.0f, float offsetY = 0.0f)
  {
    if (!this._flickerAnim.IsAnimating ? this._visible : this._visible && this._flickerAnim.IsFlickerVisible)
    {
      ...
      rect.x += offsetX;
      rect.y += offsetY;
      if (GUI.Button(rect, this._content, this._style) && this.OnClick != null)
        this.OnClick();
  ...
  {
    get => this.IsUsingGuiContentBounds ? this._style.CalcSize(this._content) : this._guiBounds;
    set => this._guiBounds = value;
  }
